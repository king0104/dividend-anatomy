...
## 후속 — Public 전환 + 네이티브 arm64 러너 (2026-08-25)

빌드 캐시로도 QEMU 에뮬레이션 자체의 느림은 안 없어졌다(8~10분대
그대로). 저장소를 private으로 유지할 이유(소스 비공개)보다 빌드
속도가 더 급해져서, 저장소를 public으로 전환하고
`runs-on: ubuntu-24.04-arm`(네이티브, QEMU 불필요)으로 바꿨다.

전환 전에 git 히스토리 전체(`git log --all -p`)를 실제 비밀번호·토큰
문자열로 grep해 `k8s/secret.yaml`/`.env`가 한 번도 커밋된 적 없음을
확인한 뒤 진행했다 — public 전환은 히스토리 전체가 그대로 공개되는
되돌리기 어려운 작업이라 사전 확인이 필요했다.

- Public 저장소는 호스티드 러너(arm64 포함)가 무제한 무료라, 이번
  전환으로 "private 무료 한도 안에서 arm64가 되는지" 자체가 더 이상
  고민할 필요 없는 질문이 됐다.
- GHCR 이미지는 애초에 public이었으니 변경 없음.
- 소스가 공개되는 트레이드오프는 있지만, 이 프로젝트 자체가 포트폴리오
  공개 목적이라 감수할 만하다고 판단.
